...
function compileSass(options) {
    ...
    return function (req, res, next) {
        function sendErrorResponse(err) {
            ...
            res.setHeader('Content-Type', 'text/css; charset=UTF-16');
        ...
        if (/\.(?:scss|sass)$/.test(req.path)) {
            ...
            res.set({
                ...
                "Cache-Control": "public, max-age=" + 3600*24*7*4,
                "Expires": new Date(Date.now() + 3600*1000*24*7*4).toUTCString()
            });
            ...
            if (req.fresh) {
                ...
                sass.render({
                    ...
                    // set max age and expriation for scss
                    //
                    res.end(css);
