...
global.pathTo = {
    view(type = "layouts", name ="") {
        let nameS = (name.slice(1) || "index").replace("/", "_")
        return path.join(__dirname, "..", "..","app","components", type, "views", nameS)
    },
    assets(type = "js", name ="elium.js") {
        return path.join(__dirname, "..", "..","app","assets", type, name)
    },
    model (type) {
        return path.join(__dirname, "..", "..","app","components", type, "model", "index.js")
    },
    route (type) {
        return path.join(__dirname, "..", "..","app","components", type, "routes", "index.js")
    },
    app (name) {
